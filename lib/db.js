...
export function findWorkspaceForUser({ userId = null, email = null, workspaceId = null } = {}) {
  if (workspaceId) {
    const workspaceById = getWorkspaceById(workspaceId);
    if (workspaceById) return workspaceById;
  }

  if (userId) {
    const workspaceByOwner = getWorkspaceByOwnerId(userId);
    if (workspaceByOwner) return workspaceByOwner;

    const userById = getUserById(userId);
    if (userById?.workspaceId) {
      const workspaceByUser = getWorkspaceById(userById.workspaceId);
      if (workspaceByUser) return workspaceByUser;
    }
  }

  if (email) {
    const userByEmail = getUserByEmail(email);
    if (userByEmail?.workspaceId) {
      const workspaceByUserEmail = getWorkspaceById(userByEmail.workspaceId);
      if (workspaceByUserEmail) return workspaceByUserEmail;
    }

    const workspaceByEmail = getWorkspaceByOwnerEmail(email);
    if (workspaceByEmail) return workspaceByEmail;
  }
