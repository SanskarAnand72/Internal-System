...
export async function getGoogleClient() {
  ...
  console.log("[GoogleClient] claims", {
    sessionUserId: session?.user?.id || null,
    sessionEmail: session?.user?.email || null,
  });
  const resolvedWorkspace = session?.user?.email
    ? findWorkspaceForUser({ email: session.user.email })
    : null;
  ...
  console.log("[GoogleClient] oauthSetup", {
    setCredentialsWillRun: true,
    refreshAccessTokenType: typeof oauth2Client.refreshAccessToken,
  });

  const originalRefreshAccessToken = oauth2Client.refreshAccessToken?.bind(oauth2Client);
  if (originalRefreshAccessToken) {
    oauth2Client.refreshAccessToken = async (...args) => {
      console.log("[GoogleClient] refreshAccessToken:start", {
        workspaceId: workspace.id,
        hasAccess,
        hasRefresh,
        expiresAt,
      ...
      try {
        const result = await originalRefreshAccessToken(...args);
        console.log("[GoogleClient] refreshAccessToken:success", {
          workspaceId: workspace.id,
          result: result?.tokens || result || null,
        });
        return result;
      } catch (error) {
        console.error("[GoogleClient] refreshAccessToken:error", {
          workspaceId: workspace.id,
  ...
  console.log("[GoogleClient] setCredentials:before", {
    workspaceId: workspace.id,
    credentials: {
  ...
  console.log("[GoogleClient] setCredentials:after", {
    workspaceId: workspace.id,
    hasCredentials: !!oauth2Client.credentials,
    credentials: oauth2Client.credentials || null,
  });

  // Auto-refresh: when googleapis refreshes the access token, persist it to DB
  oauth2Client.on("tokens", (newTokens) => {
    console.log("[GoogleClient] tokens event", {
      workspaceId: workspace.id,
      newTokens,
    });
...
export async function getCurrentWorkspace() {
  ...
  return findWorkspaceForUser({ email: session.user.email || null });
}
